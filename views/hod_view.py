...
def render_hod_view(df_encounters, df_pharmacy, df_lab):
    ...
    st.caption("Detailed departmental workflow tracking for Records, Pharmacy Inventory, and Diagnostic Laboratory Units")
    ...
    if selected_unit == "Information & Records":
        ...
        narrative_html = (
            ...
            f"Paper fallback charts were kept to <b>{paper_fallback:,} instances</b>. "
            f"Overall EMR data completeness reached <b>{completeness}</b>."
        )
    ...
    # ---------------------------------------------------------
        ...
        stockout_risk_items = 2  # Operational alert metric
        ...
        c2.metric("Total Medication Units", f"{items_disp:,}")
    ...
    # ---------------------------------------------------------
        ...
        st.subheader("Diagnostic Test Volume Distribution")
        df_test = h_lab['Test_Name'].value_counts().reset_index() if not h_lab.empty else pd.DataFrame()
        if not df_test.empty:
            df_test.columns = ['Test_Name', 'Count']
            # Expanded Donut Chart with Legends
            fig_lab = px.pie(df_test, names='Test_Name', values='Count', hole=0.5, color_discrete_sequence=px.colors.qualitative.Set3)
            fig_lab.update_traces(textinfo='percent+label')
